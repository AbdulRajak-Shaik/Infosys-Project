...
import { getCurrentWeather, getWeatherForecast, getPredictionHistory, saveLocalPrediction, getHistoryDetail, submitFeedback, getCurrentUser, getNotifications, markNotificationRead, markAllNotificationsRead } from '../services/api'
...
export function Feedback({ role, onNavigate }: { role?: string, onNavigate?: (page: string) => void }) {
  ...
  useEffect(() => {
    try {
      const saved = JSON.parse(localStorage.getItem('agroai_feedbacks') || '[]')
      if (Array.isArray(saved)) setLocalFeedbacks(saved)
    } catch {}
  }, [])

  const avgRating = localFeedbacks.length > 0 
    ? (localFeedbacks.reduce((acc, curr) => acc + curr.rating, 0) / localFeedbacks.length).toFixed(1)
    : '0.0'
  const totalReceived = localFeedbacks.length
  const uxCount = localFeedbacks.filter(f => f.category === 'ux').length
  const satisfaction = localFeedbacks.length > 0
    ? Math.round((localFeedbacks.filter(f => f.rating >= 4).length / localFeedbacks.length) * 100)
    : 0

  if (role === 'admin') {
    const feedbackList = [
      { id: 'FB-101', user: 'Gurpreet Singh', initials: 'GS', cropFocus: 'Wheat & Rice', date: 'July 26, 2026', category: 'Crop Recommendation', rating: 5, comment: 'The NPK recommendation for my sandy soil was 100% accurate. Yield increased by 18% compared to last season.', adminResponse: 'Thank you Gurpreet!\n\nGlad to hear the crop advisory boosted your harvest.', helpfulCount: 12 },
      { id: 'FB-102', user: 'Amit Sharma', initials: 'AS', cropFocus: 'Sugarcane', date: 'July 25, 2026', category: 'Weather', rating: 3, comment: 'Weather forecast was delayed by a few hours during the monsoon.', adminResponse: '', helpfulCount: 4 },
      { id: 'FB-103', user: 'Sunita Meena', initials: 'SM', cropFocus: 'Vegetables', date: 'July 22, 2026', category: 'General', rating: 4, comment: 'Good app but needs more regional languages like Marwari for Rajasthan.', adminResponse: 'Thanks for the suggestion, Sunita. We are actively working on adding Marwari support in our Q4 update.', helpfulCount: 28 },
    ]
    ...
    const toggleReply = (id: string) => setAdminReplyState(prev => ({ ...prev, [id]: !prev[id] }))
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
            <div className="flex items-center gap-6">
              <div className="flex flex-col items-center">
                <h3 className="text-5xl font-bold text-text-primary mb-2">4.8</h3>
              ...
              <div className="grid grid-cols-2 gap-x-8 gap-y-4">
                <div>
                  <p className="text-2xl font-bold text-text-primary">384</p>
            ...
            <Card key={fb.id} className="p-6 transition-all duration-300 hover:shadow-md hover:border-text-muted/30">
              ...
              <div className="pl-16 space-y-4">
                ...
                <div className="flex items-center gap-2 pt-2">
                  ...
                  {fb.adminResponse && (
                    <>
                      <Button variant="outlined" size="sm">Edit Response</Button>
                      <Button variant="outlined" size="sm" className="!text-error !border-error/30 hover:!bg-error/10">Delete Response</Button>
                    </>
                  )}
                  <Button variant="outlined" size="sm" icon={<CheckCircle2 size={14} />}>Mark as Resolved</Button>
                </div>
