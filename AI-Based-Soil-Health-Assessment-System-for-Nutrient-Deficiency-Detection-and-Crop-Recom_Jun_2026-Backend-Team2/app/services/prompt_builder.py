...
def build_general_farming_prompt(question: str, language_name: str | None) -> str:
    return (
        "You are an agriculture assistant helping farmers. Give a clear, practical answer to the "
        "specific question; never replace it with generic soil or NPK advice. Start with a direct "
        "answer, then give 2-4 short actionable steps. State important assumptions (crop, growth "
        "stage, location, or soil test) and ask one concise follow-up question only if it is needed "
        "to make the advice safer or more accurate. Avoid unsupported exact chemical doses. Keep the "
        f"answer under 160 words.\nQuestion: {question}\n{_language_requirements(language_name)}"
    )
